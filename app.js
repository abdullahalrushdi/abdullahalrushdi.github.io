...
function connectDialog(triggerId,dialogId){const trigger=$(triggerId),contactDialog=$(dialogId);trigger.addEventListener('click',event=>{event.preventDefault();contactDialog.showModal();});contactDialog.querySelector('.contact-close').addEventListener('click',()=>contactDialog.close());contactDialog.addEventListener('click',event=>{if(event.target===contactDialog)contactDialog.close();});contactDialog.addEventListener('close',()=>trigger.focus({preventScroll:true}));contactDialog.querySelectorAll('.contact-option').forEach(option=>option.addEventListener('click',()=>contactDialog.close()));}
connectDialog('whatsapp','whatsapp-dialog');
connectDialog('call','call-dialog');
